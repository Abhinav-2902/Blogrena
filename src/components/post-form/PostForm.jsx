...
const PostForm = ({post}) => {
    const {register, handleSubmit, watch, setValue, 
        ...
        const submit = async(data)=>{
            if(post){
                const file = data.image[0] ? objService.uploadFile(data.image[0])
                :null
            
            if(file){
                objService.deleteFile(post.featuredImage)
            }
            const dbPost = await objService.updatePost(
                post.$id,{
                    ...data,
           ...
           //TODO: update this area to check if file exists
            if(file){
                const fileId = file.$id
                data.featuredImage = fileId
                const dbPost = await objService.
                createPost({
                    ...data,
                    userId: userData.$id,
                })
